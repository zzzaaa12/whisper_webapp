        ...
        for line in lines:
            ...
            if stripped.startswith('>'):
                ...
                if not in_list or list_type != 'ul':
                    ...
                    in_list = True
                    list_type = 'ul'
                html_lines.append(f'<li style="margin: 8px 0; line-height: 1.8; list-style-type: none;">▸ "{quote_text}"</li>')
                continue
            ...
            # 處理縮排的子項目（第二層使用 ▸ 實心三角形）- 必須在無序列表之前檢查
            if (line.startswith('   ') or line.startswith('\t')) and in_list:
                sub_content = stripped
                if sub_content.startswith('- '):
                    sub_content = sub_content[2:]
                elif sub_content.startswith('• '):
                    sub_content = sub_content[2:]
                # 使用 ▸ 實心三角形區分第一層
                html_lines.append(f'<li style="margin: 6px 0; margin-left: 24px; line-height: 1.7; list-style-type: none;">▸ {sub_content}</li>')
                continue
